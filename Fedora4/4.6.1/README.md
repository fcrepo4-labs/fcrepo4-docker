...
2. `docker run -it -p 8080:8080 -p 9080:9080 -d yinlinchen/fcrepo4-docker:4.6.1`
...
* Ubuntu 14.04 64-bit machine with: 
  ...
  * [Fedora 4.6.1](https://wiki.duraspace.org/display/FF/Downloads) at [http://localhost:8080/fcrepo](http://localhost:8080/fcrepo)
...
```
docker build -t="4.6.1-default" .
```

To enable Fedora 4 with WebAC enabled.
```
docker build --build-arg FedoraConfig=webac- --build-arg ModeshapeConfig=servlet-auth -t="4.6.1-webac" .
```
...
```
docker build --build-arg FedoraConfig=audit- -t="4.6.1-audit" .
```

To enable Fedora 4 with WebAC and Audit capability.
```
docker build --build-arg FedoraConfig=webac-audit- --build-arg ModeshapeConfig=servlet-auth -t="4.6.1-webac-audit" .
```
